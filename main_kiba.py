# 1. IMPORTS
import pandas as pd
import torch
import torch.nn as nn
import numpy as np
import time
from torch.utils.data import Dataset, DataLoader
from sklearn.model_selection import train_test_split
from sklearn.metrics import mean_squared_error, r2_score
from torch.amp import autocast, GradScaler

start_time = time.time()

device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

scaler = GradScaler(device="cuda")

# 2. CI FUNCTION
def concordance_index(y_true, y_pred):
    n = 0
    h_sum = 0
    for i in range(len(y_true)):
        for j in range(i + 1, len(y_true)):
            if y_true[i] != y_true[j]:
                n += 1
                if (y_pred[i] > y_pred[j] and y_true[i] > y_true[j]) or \
                   (y_pred[i] < y_pred[j] and y_true[i] < y_true[j]):
                    h_sum += 1
                elif y_pred[i] == y_pred[j]:
                    h_sum += 0.5
    return h_sum / n if n > 0 else 0

# 3. LOAD DATASET
df = pd.read_csv("kiba_processed_300_add_range.csv")
df = df[['compound_iso_smiles', 'target_sequence', 'affinity']]

print("\nDATASET INFO")
print("Total samples:", len(df))
print("Avg drug length:", df['compound_iso_smiles'].apply(len).mean())
print("Avg protein length:", df['target_sequence'].apply(len).mean())

# 4. NORMALIZE TARGET
mean = df['affinity'].mean()
std = df['affinity'].std()
df['affinity'] = (df['affinity'] - mean) / std

# 5. VOCAB
drug_chars = set("".join(df['compound_iso_smiles']))
protein_chars = set("".join(df['target_sequence']))

drug2idx = {c: i+1 for i, c in enumerate(drug_chars)}
drug2idx['<pad>'] = 0

protein2idx = {c: i+1 for i, c in enumerate(protein_chars)}
protein2idx['<pad>'] = 0

drug_vocab_size = len(drug2idx)
protein_vocab_size = len(protein2idx)

MAX_DRUG_LEN = 80
MAX_PROTEIN_LEN = 150

# 6. ENCODING
def encode_drug(seq):
    seq = [drug2idx.get(c, 0) for c in seq]
    seq = seq[:MAX_DRUG_LEN]
    seq += [0] * (MAX_DRUG_LEN - len(seq))
    return torch.tensor(seq)

def encode_protein(seq):
    seq = [protein2idx.get(c, 0) for c in seq]
    seq = seq[:MAX_PROTEIN_LEN]
    seq += [0] * (MAX_PROTEIN_LEN - len(seq))
    return torch.tensor(seq)

# 7. DATASET
class DTADataset(Dataset):
    def __init__(self, df):
        self.df = df.reset_index(drop=True)

    def __len__(self):
        return len(self.df)

    def __getitem__(self, idx):
        row = self.df.iloc[idx]
        return (
            encode_drug(row['compound_iso_smiles']),
            encode_protein(row['target_sequence']),
            torch.tensor(row['affinity'], dtype=torch.float)
        )

# 8. SPLIT
train_df, temp_df = train_test_split(df, test_size=0.2, random_state=42)
val_df, test_df = train_test_split(temp_df, test_size=0.5, random_state=42)

print("\nDATA SPLIT")
print(f"Train: {len(train_df)} | Val: {len(val_df)} | Test: {len(test_df)}")

train_loader = DataLoader(DTADataset(train_df), batch_size=128, shuffle=True)
val_loader = DataLoader(DTADataset(val_df), batch_size=128)
test_loader = DataLoader(DTADataset(test_df), batch_size=128)

# 9. MODEL
class DTA_Model(nn.Module):
    def __init__(self):
        super().__init__()

        self.drug_embedding = nn.Embedding(drug_vocab_size, 128)
        self.protein_embedding = nn.Embedding(protein_vocab_size, 128)

        self.drug_conv1 = nn.Conv1d(128, 128, 3, padding=1)
        self.drug_conv2 = nn.Conv1d(128, 128, 5, padding=2)
        self.drug_conv3 = nn.Conv1d(128, 128, 7, padding=3)
        self.drug_conv4 = nn.Conv1d(128, 128, 9, padding=4)

        self.protein_conv1 = nn.Conv1d(128, 128, 3, padding=1)
        self.protein_conv2 = nn.Conv1d(128, 128, 5, padding=2)
        self.protein_conv3 = nn.Conv1d(128, 128, 7, padding=3)
        self.protein_conv4 = nn.Conv1d(128, 128, 9, padding=4)

        self.query = nn.Linear(128, 128)
        self.key = nn.Linear(128, 128)
        self.value = nn.Linear(128, 128)

        self.attn_pool = nn.Linear(128, 1)

        self.feature_fc = nn.Sequential(
            nn.Linear(128 * 3, 256),
            nn.BatchNorm1d(256),
            nn.ReLU(),
            nn.Dropout(0.3)
        )

        self.calibration = nn.Sequential(
            nn.Linear(256, 64),
            nn.ReLU(),
            nn.Linear(64, 1)
        )

    def encode(self, x, emb, c1, c2, c3, c4):
        x = emb(x).permute(0, 2, 1)
        x = torch.relu(c1(x))
        x = torch.relu(c2(x))
        x = torch.relu(c3(x))
        x = torch.relu(c4(x))
        return x.permute(0, 2, 1)

    def cross_attention(self, d, p):
        Q = self.query(d)
        K = self.key(p)
        V = self.value(p)
        attn = torch.softmax(Q @ K.transpose(-2, -1) / (128**0.5), dim=-1)
        return attn @ V

    def pool(self, x):
        w = torch.softmax(self.attn_pool(x), dim=1)
        return (x * w).sum(dim=1)

    def forward(self, drug, protein):
        d = self.encode(drug, self.drug_embedding,
                        self.drug_conv1, self.drug_conv2,
                        self.drug_conv3, self.drug_conv4)

        p = self.encode(protein, self.protein_embedding,
                        self.protein_conv1, self.protein_conv2,
                        self.protein_conv3, self.protein_conv4)

        c = self.cross_attention(d, p)

        combined = torch.cat([self.pool(d), self.pool(p), self.pool(c)], dim=1)

        return self.calibration(self.feature_fc(combined))

# 10. TRAINING
model = DTA_Model().to(device)
optimizer = torch.optim.Adam(model.parameters(), lr=0.0002)
loss_fn = nn.MSELoss()

print("\nMODEL PARAMETERS:", sum(p.numel() for p in model.parameters()))

best_loss = float('inf')
best_epoch = 0

for epoch in range(45):
    model.train()
    total_loss = 0

    for drug, protein, label in train_loader:
        drug, protein, label = drug.to(device), protein.to(device), label.to(device)

        optimizer.zero_grad()

        with autocast("cuda"):
            output = model(drug, protein).squeeze()
            loss = loss_fn(output, label)

        scaler.scale(loss).backward()
        scaler.step(optimizer)
        scaler.update()

        total_loss += loss.item()

    model.eval()
    val_loss = 0
    with torch.no_grad():
        for drug, protein, label in val_loader:
            drug, protein, label = drug.to(device), protein.to(device), label.to(device)
            val_loss += ((model(drug, protein).squeeze() - label) ** 2).mean().item()

    print(f"Epoch {epoch+1} | Train: {total_loss:.4f} | Val: {val_loss:.4f}")

    if val_loss < best_loss:
        best_loss = val_loss
        best_epoch = epoch + 1
        torch.save(model.state_dict(), "best_model.pth")

print(f"\nBEST EPOCH: {best_epoch} | BEST VAL LOSS: {best_loss:.4f}")

model.load_state_dict(torch.load("best_model.pth"))

# 11. TEST + TIMING
model.eval()
y_true, y_pred = [], []

start_test = time.time()

with torch.no_grad():
    for drug, protein, label in test_loader:
        drug, protein = drug.to(device), protein.to(device)
        pred = model(drug, protein).squeeze().cpu().numpy()
        y_pred.extend(pred)
        y_true.extend(label.numpy())

end_test = time.time()

y_pred = [p * std + mean for p in y_pred]
y_true = [t * std + mean for t in y_true]

mse = mean_squared_error(y_true, y_pred)
r2 = r2_score(y_true, y_pred)
ci = concordance_index(y_true, y_pred)

# 12. FINAL OUTPUT
print("\nFINAL MODEL PERFORMANCE")
print(f"MSE: {mse:.4f}")
print(f"R2: {r2:.4f}")
print(f"CI: {ci:.4f}")

#  Baseline (DeepDTA)
baseline_mse = 0.201
baseline_r2 = 0.671
baseline_ci = 0.850

print("\nCOMPARISON WITH BASELINE (KIBA)")
print(f"Baseline -> MSE {baseline_mse} | R2 {baseline_r2} | CI {baseline_ci}")
print(f"Your Model -> MSE {mse:.4f} | R2 {r2:.4f} | CI {ci:.4f}")

#  Improvements
mse_imp = ((baseline_mse - mse) / baseline_mse) * 100
r2_imp = ((r2 - baseline_r2) / baseline_r2) * 100
ci_imp = ((ci - baseline_ci) / baseline_ci) * 100

print("\nIMPROVEMENT OVER BASELINE")
print(f"MSE Improvement: {mse_imp:.2f}%")
print(f"R2 Improvement: {r2_imp:.2f}%")
print(f"CI Improvement: {ci_imp:.2f}%")

print(f"\nTraining Time: {(time.time() - start_time)/60:.2f} minutes")
print(f"Inference Time: {(end_test - start_test):.2f} seconds")

print("\nMODEL SUMMARY")
print("Deep CNN + Cross Attention + Attention Pooling + Calibration Head")
